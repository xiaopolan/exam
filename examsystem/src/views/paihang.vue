	...
	export default {
		...
		methods: {
			...
			sentMsg() {
				...
				var that = this;
				var outText = "";
				for (var i = 0; i < that.sensitiveWordsList.length; i++) {
					var pattern = new RegExp(that.sensitiveWordsList[i], "g");
					if (i == 0) {
						outText = that.message.replace(pattern, "**");
					} else {
						outText = outText.replace(pattern, "**");
					}
				};
				that.message = outText
